...
const DefaultService = require("../services/display_bom.service");
const {
  createSchema,
  updateSchema,
} = require("../helpers/validations/display_bom.validation");
...
exports.create = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("display_bom.suc.create"),
      data: defaultService,
    });
  } catch (err) {
    logger.error(req.path);
...
exports.search = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("display_bom.suc.search"),
      data: data,
      pagination: {
        page_num: pageNum,
        page_limit: pageLimit,
...
exports.update = async (req, res) => {
  try {
    ...
    if (!defaultService) {
      return res.status(400).send({
        status: "error",
        message: lang.t("display_bom.err.not_exists"),
      });
    }
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("display_bom.suc.update"),
...
exports.delete = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("display_bom.suc.delete"),
