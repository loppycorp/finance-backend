...
const DefaultService = require("../services/monthly_utilization.service");
const {
  createSchema,
  updateSchema,
} = require("../helpers/validations/monthly_utilization.validation");
...
exports.create = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("monthly_utilization.suc.create"),
      data: defaultService,
    });
  } catch (err) {
    logger.error(req.path);
...
exports.search = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("monthly_utilization.suc.search"),
      data: data,
      pagination: {
        page_num: pageNum,
        page_limit: pageLimit,
...
exports.update = async (req, res) => {
  try {
    ...
    if (!defaultService) {
      return res.status(400).send({
        status: "error",
        message: lang.t("monthly_utilization.err.not_exists"),
      });
    }
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("monthly_utilization.suc.update"),
...
exports.delete = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("monthly_utilization.suc.delete"),
