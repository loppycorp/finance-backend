...
const DefaultService = require("../services/posting_key.service");
const { createSchema, updateSchema } = require("../helpers/validations/posting_key.validation");
...
exports.create = async (req, res) => {
  try {
    ...
    if (code) {
      return res.status(400).send({
        status: 'error',
        message: lang.t('posting key code already exists')
      });
    }
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("posting_key.suc.create"),
      data: defaultService,
    });
  } catch (err) {
...
exports.search = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("posting_key.suc.search"),
      data: data,
      pagination: {
        page_num: pageNum,
...
exports.update = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("posting_key.suc.update"),
...
exports.delete = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      status: "success",
      message: lang.t("posting_key.suc.delete"),
