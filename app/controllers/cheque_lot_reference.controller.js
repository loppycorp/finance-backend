...
const DefaultService = require('../services/cheque_lot_reference.service');
const { paramsSchema } = require('../helpers/validations/common.validation');
const { createSchema, updateSchema } = require('../helpers/validations/cheque_lot_reference.validation');
...
exports.get = async (req, res) => {
    try {
        ...
        if (!defaultService) {
            return res.status(400).send({
                status: 'error',
                message: lang.t('cheque_lot.err.not_exists')
            });
        }

        return res.status(200).send({
            status: 'success',
            message: lang.t('cheque_lot.suc.read'),
            data: defaultService
        });
    } catch (err) {
        logger.error(req.path);
...
exports.search = async (req, res) => {
    try {
        ...
        return res.status(200).send({
            status: "success",
            message: lang.t("cheque_lot.suc..search"),
            data: data,
            pagination: {
                page_num: pageNum,
...
exports.findAllById = async (req, res) => {
    try {
        ...
        const { data, total } = await DefaultService.getAllById(query, params.id);
...
exports.update = async (req, res) => {
    try {
        ...
        return res.status(200).send({
            status: "success",
            message: lang.t("cheque_lot.suc.update"),
...
exports.delete = async (req, res) => {
    try {
        ...
        return res.status(200).send({
            status: "success",
            message: lang.t("cheque_lot.suc.delete"),
