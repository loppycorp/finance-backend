...
const defaultSchema = Joi.object({
    ...
    basic_data: {
        names: {
            ...
            description: Joi.string().trim().max(LIMIT_DEFAULT_CHAR_L),
        },
        basic_data: {
            user_responsible: Joi.string().trim().required().hex().max(24).allow('', null),
            ...
            business_area: Joi.number().strict().required().allow(null),
            functional_area: Joi.number().strict().required().allow(null),
            currency: Joi.string().trim().required().hex().max(24),
            profit_center: Joi.string().trim().required().hex().max(24),
        },
    },
});
module.exports = {
